...
var (
	errAdminCLIExclusiveFlags = errors.New("use only one of --add-admin, --remove-admin, or --show-admin")
...
func handleAdminCLI(args []string, keyringStore credentialStore, reader *bufio.Reader, out io.Writer) (bool, error) {
	...
	switch command {
	case "add":
		return true, handleAddAdminCLI(keyringStore, reader, out)
	case "remove":
...
func handleAddAdminCLI(keyringStore credentialStore, reader *bufio.Reader, out io.Writer) error {
	...
	if err := keyringStore.SetKeyValue(keyringAdminUsername, username); err != nil {
		return fmt.Errorf("failed to store admin username in keystore: %w", err)
	}

	if err := keyringStore.SetKeyValue(keyringAdminPassword, hashedPassword); err != nil {
		_ = keyringStore.DeleteKeyValue(keyringAdminUsername)

		return fmt.Errorf("failed to store admin password in keystore: %w", err)
	}
	...
	fmt.Fprintln(out, "Admin credentials stored in keystore.")
