...
func (r *packageRoutes) build(c *gin.Context) {
	...
	reader, filename, err := r.t.BuildPackage(c.Request.Context(), req, middleware.TenantID(c))
	if err != nil {
		if errors.Is(err, packaging.ErrTokenTTLTooLong) || errors.Is(err, packaging.ErrTokenTTLInvalid) {
			ErrorResponse(c, errValidationPackage.Wrap("build", "tokenTtl", err))

			return
		}

		if errors.Is(err, packaging.ErrTokenModeUnsupported) {
			ErrorResponse(c, errValidationPackage.Wrap("build", "auth.mode", err))
