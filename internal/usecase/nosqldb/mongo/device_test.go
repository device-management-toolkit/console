...
func TestDeviceRepo_GetByID_IdentityColumns(t *testing.T) {
	...
	md.AddResponses(findResponse(
		...
		bson.D{
			...
			{Key: "lastupdate", Value: "2026-06-19T09:30:00Z"},
	...
	require.Equal(t, "2026-06-19T09:30:00Z", got.LastUpdate)
...
func TestDeviceRepo_Update_OmitsImmutableIdentityColumns(t *testing.T) {
	...
	// lastupdate is server-refreshed on every record edit, so it must be written.
	require.Contains(t, set, "lastupdate", "lastupdate must be refreshed via Update")
}

// TestDeviceRepo_UpdateLastSeen_OmitsLastUpdate: the heartbeat path writes only
// lastseen, never lastupdate.
func TestDeviceRepo_UpdateLastSeen_OmitsLastUpdate(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, repo.UpdateLastSeen(context.Background(), "g1"))

	cmd, found := cc.byName("update")
	require.True(t, found, "expected an update command to be sent")

	set := updateSetKeys(t, cmd)
	require.Contains(t, set, "lastseen")
	require.NotContains(t, set, "lastupdate", "UpdateLastSeen must not touch lastupdate")
}
