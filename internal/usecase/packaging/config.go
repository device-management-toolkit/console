...
func renderConfig(req dto.PackageRequest, in configInputs) ([]byte, error) {
	...
	// authModeNone writes no credentials; the operator supplies them on the device.
	...
	case commandActivate:
		if req.Domain != "" {
			cfg.Activate.URL = fmt.Sprintf(exportPathDomainFmt, in.ExportBase, url.PathEscape(req.Profile), url.QueryEscape(req.Domain))
		} else {
