...
type configFile struct {
	...
	// rpc-go resolves config keys by flag name: --tenantid and --password.
	TenantID    string `yaml:"tenantid"`
	AMTPassword string `yaml:"password"`
	// Omitted when empty: rpc-go reads config after env, so an empty key would
	// override AUTH_TOKEN / AUTH_USERNAME / AUTH_PASSWORD set on the device.
...
type configInputs struct {
	...
	TenantID        string // tenant the package is scoped to; empty is the default tenant
	SkipCertCheck   bool   // server presents a self-signed cert rpc-go cannot chain
}
...
func renderConfig(req dto.PackageRequest, in configInputs) ([]byte, error) {
	...
	cfg.TenantID = in.TenantID
	cfg.SkipCertCheck = in.SkipCertCheck

	// Mode "none" writes no credentials; the operator supplies them on the device.
	...
	case commandActivate:
		cfg.Activate.URL = fmt.Sprintf(exportPathFmt, in.ExportBase, url.PathEscape(req.Profile))
		if req.Domain != "" {
			cfg.Activate.URL += "?domainName=" + url.QueryEscape(req.Domain)
		}
	...
	out, err := yaml.Marshal(cfg) //nolint:gosec // G117: config intentionally serializes credential fields (password, auth-token, etc.) — the caller is responsible for protecting the resulting bytes
	if err != nil {
