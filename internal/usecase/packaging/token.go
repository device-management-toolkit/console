...
var (
	...
	// ErrTokenModeUnsupported indicates token auth under OIDC, where Console-minted tokens are not accepted.
	ErrTokenModeUnsupported = errors.New("token auth mode is not supported when OIDC is configured")
)
