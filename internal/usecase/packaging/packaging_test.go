...
func TestBuildPackageBothOffline(t *testing.T) {
	...
	winPath := filepath.Join(tmp, "v3.0.1", "rpc-go_Windows_x86_64.exe")
...
func TestListVersionsFetchedThenLocal(t *testing.T) {
	...
	body := `[
	  {"tag_name":"v3.1.0","assets":[{"name":"rpc-go_Linux_x86_64.tar.gz","browser_download_url":"http://x/a"}]},
	  {"tag_name":"v3.0.1","assets":[{"name":"rpc-go_Windows_x86_64.zip","browser_download_url":"http://x/b"}]}
	]`
	...
	if err := os.WriteFile(filepath.Join(tmp, "v3.0.0", "rpc-go_Linux_x86_64.tar.gz"), []byte("x"), 0o600); err != nil {
		t.Fatal(err)
	}
