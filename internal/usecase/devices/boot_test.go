...
func TestSendRemoteErase(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "eraseMask not supported by device capabilities",
			eraseMask: 4,
			manMock: func(man *mocks.MockWSMAN, man2 *mocks.MockManagement) {
				man.EXPECT().
					SetupWsmanClient(gomock.Any(), false, true).
			...
			err: devices.ValidationError{}.Wrap("SendRemoteErase", "validate erase mask", "requested erase capabilities are not supported by this device"),
		},
		{
			name:      "GetByID returns error",
			eraseMask: 0,
