...
// The CIRA socket belongs to the tenant that authenticated it, so a caller from
// another tenant must not be handed the connection. Mirrors MPS ciraMiddleware.
func TestSetupWsmanClient_CIRATenantMismatchIsRejected(t *testing.T) { //nolint:paralleltest // mutates package-level state (requestQueue, queueTickTime, connections)
	origTick := queueTickTime
	queueTickTime = 1 * time.Millisecond

	t.Cleanup(func() { queueTickTime = origTick })

	guid := "cira-tenant-mismatch-guid"
	...
	mismatched := entity.Device{
		GUID:        guid,
		MPSUsername: "mpsuser",
		TenantID:    "globex",
	}

	_, err := g.SetupWsmanClient(context.Background(), mismatched, false, false)
	require.ErrorIs(t, err, ErrCIRATenantMismatch)
	...
	_, err = g.SetupWsmanClient(context.Background(), owner, false, false)
