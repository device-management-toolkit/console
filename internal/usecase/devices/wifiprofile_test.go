...
// expectWiFiPortPresent registers the WiFi port lookup performed by the
// AddWirelessProfile/UpdateWirelessProfile guard, returning a single WiFi port
// so the operation proceeds.
func expectWiFiPortPresent(man2 *mocks.MockManagement) {
	man2.EXPECT().GetWiFiPorts().Return([]wifi.WiFiPort{{}}, nil)
}

// expectWiFiPortAbsent registers the WiFi port lookup performed by the guard,
// returning ErrNoWiFiPort so the operation fails.
func expectWiFiPortAbsent(man2 *mocks.MockManagement) {
	...
	tests := []struct {
		...
		},
		{
			...
			res: []dto.WirelessProfileResponse{{
				...
				EncryptionMethod:     "CCMP",
				Priority:             1,
				IEEE8021x: &dto.WirelessIEEE8021xResponse{
					AuthenticationProtocol: 2,
					Username:               "corp-user",
				},
			}},
...
func TestAddWirelessProfile(t *testing.T) { //nolint:gocognit // table-driven coverage for create flow branches
	...
	tests := []struct {
		...
		profile   config.WirelessProfile
		manMock   func(*mocks.MockWSMAN, *mocks.MockManagement)
		portMock  func(*mocks.MockManagement)
		repoMock  func(*mocks.MockDeviceManagementRepository)
		err       error
		errString bool
	}{
		...
		{
			name:    "no wifi port returns ErrNoWiFiPort",
			profile: profile,
			manMock: func(man *mocks.MockWSMAN, man2 *mocks.MockManagement) {
				man.EXPECT().SetupWsmanClient(gomock.Any(), gomock.Any(), false, true).Return(man2, nil)
			},
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.portMock != nil {
				tc.portMock(management)
			} else if tc.manMock != nil {
				expectWiFiPortPresent(management)
			}
