	...
	tests := []struct {
		...
		},
		{
			...
			setup: func(mockRedir *mocks.MockRedirection, mockRepo *mocks.MockDeviceManagementRepository, mockWSMAN *mocks.MockWSMAN, wg *sync.WaitGroup) {
				...
				mockRedir.EXPECT().SetupWsmanClient(gomock.Any(), gomock.Any(), true, true).Return(wsman.Messages{}, entity.Device{}, nil)
...
func TestRedirectSuccessfulFlow(t *testing.T) {
	...
	mockRedirection.EXPECT().SetupWsmanClient(gomock.Any(), *device, true, true).Return(wsman.Messages{}, *device, nil)
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMocks: func(mockRedir *mocks.MockRedirection, mockRepo *mocks.MockDeviceManagementRepository, mockWSMAN *mocks.MockWSMAN, wg *sync.WaitGroup) {
				...
				mockRepo.EXPECT().GetByGUID(gomock.Any(), testGUID).Return(device, nil)
				mockRedir.EXPECT().SetupWsmanClient(gomock.Any(), *device, true, true).Return(wsman.Messages{}, *device, nil)
